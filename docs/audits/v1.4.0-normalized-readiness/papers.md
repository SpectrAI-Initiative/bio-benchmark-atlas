...
| MoleculeNet auxiliary learning | Small molecule | 3 partial | 0 | Re-audited; keep partial | The Table 1 re-audit pinned GraphTA `v0.1.0`/commit `0eef8c9` and located the linked upstream dataset archive and pretrained checkpoint blobs. However, neither the article nor the archived GraphTA release records the content digest or immutable revision of the dataset archive and checkpoints used for the reported runs. Current public bytes therefore cannot be identified as the evaluated snapshot with high confidence. | Keep partial. Revisit only if the authors publish a manifest or versioned result bundle tying Table 1 to exact data and checkpoint artifacts. |
...
1. Select additional source-rich papers specifically for normalized runs until at least six target papers or benchmark families are backed by normalized runs.
2. Keep PPB-Affinity, MoleculeNet auxiliary learning, Crafted experiments, multiobjective protein design, CRISPRn guide design, and differential transcript usage as partial until new primary artifacts resolve their blockers.
3. Do not count a paper toward the normalized target merely because it prints a score; all normalization fields above must be supported.
